...
def proximity_to_food(level: List[str], path: List[Tuple[int, int]]) -> float:
    in_proximity = 0
    for (y, x) in path:
        for (x_mod, y_mod) in ENEMY_RADIUS:
            _x = x + x_mod
...
def food_density(level: List[str]) -> float:
    num_tiles = len(level) * len(level[0])
    return sum(sum(1 for t in r if t =='&') for r in level) / num_tiles

def linearity(level: List[str]) -> float:
    num_tiles = len(level) * len(level[0])
    count = 0
    for col in level:
        count += sum(1 for t in col if t == '^' or t =='#' or t=='*')
...
for lvl_key in data_iterator:
    ...
    V = [ 
        len(path_with_enemies) - len(path_no_enemies),                # path difference no enemies
        len(path_with_enemies) - len(path_no_nothing),                # path difference no enemies and switches
        ...
        food_density(level),                                          # Density of food in the level
        proximity_to_food(level, path_with_enemies),
    ]
